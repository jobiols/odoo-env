...
class TestCreateTestDb(OdooEnvTestCase):
    ...
    def test_db_exists_does_not_interpolate_database_name_into_sql(self):
        ...
        El nombre se pasa como parámetro de conexión de psql (-d) y el SQL
        es un literal fijo "SELECT 1", de modo que un nombre con comillas
        nunca rompe ni inyecta SQL.
        """
        options = MockArgs(client="test_client")
        oe = OdooEnv(options)
        ...
        self.assertEqual(sql_arg, "SELECT 1")
        self.assertIn("-d", cmd)
        d_index = cmd.index("-d")
        self.assertEqual(cmd[d_index + 1], "weird'name")
