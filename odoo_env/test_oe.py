...
from odoo_env.constants import DBTOOLS_IMAGE, WDB_IMAGE_DEFAULT, WDB_IMAGE_16, WDB_IMAGE_NEW, BASE_DIR
...
TEST2E_CLIENT_MANIFEST = {
    ...
    'git-repos': [
        'https://github.com/jobiols/odoo-enterprise.git'
    ]
}
...
class TestRepository(unittest.TestCase):
    ...
    def test_install(self):
        ...
        self.assertEqual(cmds[2].command, ["sudo", "mkdir", "-p", "{}odoo-9.0/test_client/postgresql".format(BASE_DIR)])
    ...
    def test_install2(self):
        ...
        self.assertEqual(cmds[0].command, ["sudo", "mkdir", "-p", BASE_DIR])
        self.assertEqual(cmds[2].command, ["sudo", "mkdir", "-p", "{}odoo-9.0/test2_client/postgresql".format(BASE_DIR)])

    def test_install2_enterprise(self):
        self.mock_get_manifest.side_effect = lambda path: TEST2E_CLIENT_MANIFEST
        options = {"debug": True, "no-repos": False, "nginx": True, "extract_sources": False}
        oe = OdooEnv(options)
        cmds = oe.install("test2e_client")
        self.assertEqual(cmds[0].command, ["sudo", "mkdir", "-p", BASE_DIR])
        self.assertEqual(cmds[2].command, ["sudo", "mkdir", "-p", "{}odoo-9.0e/test2e_client/postgresql".format(BASE_DIR)])
    ...
    def test_qa(self):
        ...
        # Order: run, --rm, -it, --network, volumes, env, links, image, stop-after-init, log-level, test-enable, extra_args
        command = [
            "sudo", "docker", "run", "--rm", "-it",
            "--network", "odoo-net",
            "-v", f"{BASE_DIR}odoo-9.0/test_client/config:/opt/odoo/etc/:rw",
            "-v", f"{BASE_DIR}odoo-9.0/test_client/data_dir:/opt/odoo/data:rw",
            "-v", f"{BASE_DIR}odoo-9.0/test_client/log:/var/log/odoo:rw",
            "-v", f"{BASE_DIR}odoo-9.0/test_client/sources:/opt/odoo/custom-addons:rw",
            "-v", f"{BASE_DIR}odoo-9.0/test_client/backup_dir:/var/odoo/backups/:rw",
    ...
    def test_pull_images(self):
        ...
        self.assertEqual(cmds[0].command, ["sudo", "docker", "pull", "jobiols/odoo-jeo:9.0"])
    ...
    def test_restore(self):
        ...
        command = [
            "sudo", "docker", "run", "--rm",
            "--network", "odoo-net",
            "-v", f"{BASE_DIR}odoo-9.0/test_client/backup_dir/:/backup:rw",
            "-v", f"{BASE_DIR}odoo-9.0/test_client/data_dir/filestore:/filestore:rw",
            "-e", "NEW_DBNAME=client_prod",
            "-e", "ZIPFILE=bkp.zip",
            "-e", "DEACTIVATE=True",
            DBTOOLS_IMAGE
        ]
    ...
    def test_download_image_sources(self):
        self.mock_get_manifest.side_effect = lambda path: TEST_CLIENT_MANIFEST
        options = {"debug": True, "no-repos": False, "nginx": False, "extract_sources": True}
        oe = OdooEnv(options)
        cmds = oe.install("test_client")
        self.assertEqual(cmds[0].command, ["sudo", "mkdir", "-p", BASE_DIR])
        ...
        command = [
            ...
            "-v", f"{BASE_DIR}odoo-9.0/dist-packages/:/mnt/dist-packages:rw",
            "jobiols/odoo-jeo:9.0"
        ]
